    ...
    with open('Tokens.txt', "w", encoding="utf-8") as filehandle:
        for i in range(1, number_of_rows + 1):
            ...
            for j in range(len(temp_tokens)):
                ...
                if ((listitem in file.read()) and (listitem not in garbage)):
                    print("stop :" + listitem)
                    ...
                    if "&" in stem_token:
                        ...
                        add_to_index_with_stop(mazi, j + 1, i)
                        add_to_index_with_stop(mozare, j + 1, i)

                    else:
                        # filehandle.write('%s\n' % (stem_token + ":" + listitem))
                        add_to_index_with_stop(stem_token, j + 1, i)
                else:
